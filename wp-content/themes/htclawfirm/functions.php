		...
		<?php settings_fields( 'my-settings-group' ); ?>
			<div class="block-config-mymenu">
				...
				<div class="px-15" style="clear: both;">
				 <p><label for="social">Mạng xã hội</label><br/>
...
function contactForm() {
	...
	if(isset($_POST['btn-send']) ) { 
		$address_company = '626A / 2, Tổ 2, Châu Thới, phường Bình An, Thị trấn Dĩ An, Tỉnh Bình Dương, Việt Nam.';$phone_company = '1900 0316';$mail_company = 'service@martoyo.net';
	    ...
	    $kiemtra=file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=6LdoOrEZAAAAAAEU1HJYpTd_VpPmf14N_7YHUM2X&response=".$tut_captcha."&remoteip=".$_SERVER['REMOTE_ADDR']);
	    ...
	    if($kiemtra->success == false) {
	      ...
	      die();
			...
			$mail->Subject    = "Nội Dung Thông Điệp Từ Trang - ".get_bloginfo( 'name' )."";
	    ...
	    <form action="" method="post" accept-charset="utf-8">                                
	      ...
	      <div class="form-group g-recaptcha-block row align-items-center">
            ...
            <div class="col-12 col-sm-4 text-center">
            	<input type="submit" value="Gửi yêu cầu" class="btn btn-read-more text-capitalize px-3 py-2 ml-auto" name="btn-send">
            </div>
          </div>
...
function cptui_register_my_cpts() {
	 ...
	 */
	$labels = array(
		"name" => __( "News Letter", "martoyo" ),
		"singular_name" => __( "Sub Email", "martoyo" ),
	);

	$args = array(
		"label" => __( "News Letter", "martoyo" ),
		"labels" => $labels,
		"description" => "",
		"public" => true,
...
function catalog_grid($name_category = '') {
	...
	if ( $the_query->have_posts() ) {	
		$string .='<div class="row grid-item grid-2">';
		while ( $the_query->have_posts() ) {
			$the_query->the_post();			
			if ( has_post_thumbnail() ) {
				$string .= '<div class="col-12 col-sm-6 col-md-4 my-3">
		          	<a class="post-media p-2 mb-4 d-block" href="' . get_the_permalink() .'" title="' . get_the_title() .'" ><div class="vertical-img">' .get_the_post_thumbnail( get_the_id(), 'full', array( 'class' =>'img-fluid mx-auto d-block','alt' => get_the_title()) ).'</div></a>
			        <div class="post-content">
			          <h3 class="text-capitalize"><a href="' . get_the_permalink() .'" title="' . get_the_title() .'" >' .get_the_title() .'</a></h3>
			          <p>'. substr(strip_tags(get_the_content()), 0, 122) . '...</p>
			        </div>
			    </div>';
			} 
		}
				...
				$string .= '<div class="col-sm-6 col-md-4 my-3">
			      <div class="item-block item-p">
			        <div class="post-media fit-img-1 fit-img-cat mb-4">
			          <a class="inner-fit-img position-relative d-block" href="' . get_the_permalink() .'" title="' . get_the_title() .'" >' .get_the_post_thumbnail( get_the_id(), 'full', array( 'class' =>'mx-auto d-block img-fluid w-100 h-100','alt' => get_the_title()) ).'</a>
			        </div>
			        <div class="post-content">
...
function list_case($name_category = '',$postPage = 5) {
	...
	if ( $the_query->have_posts() ) {		
		...
		while ( $the_query->have_posts() ) {
	        ...
	        $string .= '<li><div class="img"><a class="wrap-fit-img d-block" href="' . get_the_permalink() .'" title="' . get_the_title() .'" >' .get_the_post_thumbnail( get_the_id(), 'full', array( 'class' =>'img-fluid mx-auto d-block','alt' => get_the_title() ) ).'</a>
...
function list_case_cat_img($name_category = '') {
	...
	if ( $the_query->have_posts() ) {		
		...
		while ( $the_query->have_posts() ) {
			...
			$thumb = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'full' );
			if ( has_post_thumbnail() ) {		
				$string .= '<div class="item-list"><a href="' . get_the_permalink() .'" title="' . get_the_title() .'"><div class="fix-img-fit mb-4">' .get_the_post_thumbnail( get_the_id(), 'full', array( 'class' =>'img-fluid mx-auto d-block w-100','alt' => get_the_title()) ).'</div>
...
function get_products_home($catalog='') {
      ...
      echo '<div cat-all>';
        foreach ( $terms as $term ) {
          if($term->parent == 0) {                 
            ...
            if($term->count > 0) {                
              echo '<div class="item-list"><a href="' . get_term_link($term->slug, $taxonomy_product) . '" title="' . $term->name .'"><div class="fix-img-fit mb-4"><img class="img-fluid mx-auto d-block w-100" src="'. $banerUrl .'" alt="'.$term->name.'"></div>
          		<div class="description"><h3 class="pb-3 mb-3">Danh mục sản phẩm</h3><div class="text">' . $term->name .'</div></div></a></div>';
            }
          }
        } 
      echo "</div>";
...
function get_products_special() {
    ...
    if($q_products->have_posts()) {
        echo '<div cat-all>';
    		...
    		//$check_special = get_post_meta($post_id,'check_special',true);
    		// if($check_special == )

	          echo '<div class="item-list"><a href="' . get_the_permalink() . '" title="' . get_the_title() .'"><div class="fix-img-fit mb-4">'.get_the_post_thumbnail($post_id, 'full', array( 'class' => 'img-fluid mx-auto d-block w-100','alt' => get_the_title() )).'</div>
          		<div class="description"><h3 class="pb-3 mb-3">Sản phẩm nổi bật</h3><div class="text">' . get_the_title() .'</div></div></a></div>';
	        }
        echo '</div>';          
    }
}
function breadcrumb() {
    ...
    if($prod_tax->name == 'products' || $prod_tax->name == 'san-pham') {echo "Sản phẩm";} 
  	$term_tax = get_term_by("slug", get_query_var("term"), get_query_var("taxonomy") );
  	if($term_tax->taxonomy == 'menu-products') {
	  	$parent = get_term($term_tax->parent, get_query_var("taxonomy"));
	  	echo "<a title='Sản phẩm' href=" . home_url() . "/san-pham >Sản phẩm</a>";
	    echo '<i class="fa fa-angle-double-right mx-1"></i>';
	    while ($parent->term_id) {
	        echo "<a title=" . $parent->name . " href=\"" . home_url() . "/menu-products/" . $parent->slug . "\">" . $parent->name . "</a>";
...
function news_footer() {
    ...
    $the_query = new WP_Query( array( 'cat' => $cat_id, 'posts_per_page' => 4  ) );
      if ( $the_query->have_posts() ) {
        $string = ''; 
        while ( $the_query->have_posts() ) {
          $the_query->the_post();
          if ( has_post_thumbnail() ) {                          
            $string .= '<a class="row mb-3 item-news" href="' . get_the_permalink() .'" title="' . get_the_title() .'">
              <div class="col-4 pr-0"><div class="position-relative wrap-fit-img">' .get_the_post_thumbnail( get_the_id(), 'full', array( 'class' =>'img-fluid','alt' => get_the_title() ) ).'
              </div></div>
              <div class="col-8">
                <div class="text-capitalize">'.get_the_title().'</div>
              </div>
            </a>';                
          } 
...
function my_category_module() {
	...
	add_action( 'menu-products_add_form_fields', 'add_category_image', 10, 2 ); 
	add_action( 'created_menu-products',  'save_category_image' , 10, 2 );
	add_action( 'menu-products_edit_form_fields',  'update_category_image' , 10, 2 );
	add_action( 'edited_menu-products',  'updated_category_image' , 10, 2 );
...
function create_posttype_sanpham() {
	...
	register_taxonomy( 'menu-products', array( 'products' ),
		array(
			'labels' => array(
				'name' => 'Taxomy Products',
				'menu_name' => 'Taxomy Products',
				'singular_name' => 'Taxomy Products',
				'all_items' => 'Taxomy Products'
			),
			'public' => true,
			'hierarchical' => true,
			'show_ui' => true,
			'rewrite' => array( 'slug' => 'menu-products', 'hierarchical' => true, 'with_front' => false ),
		)
	);
}
...
function sanpham_meta_box() {
	add_meta_box( 'sanpham-info', 'Info Product', 'sanpham_output', 'products' );
}
...
function sanpham_output($post) {  
	...
	$sort_product = get_post_meta($post->ID,'sort_product',true);

	$custom = get_post_custom($post->ID);
  	// $check_special = $custom["url"][0];
	$check_special = isset( $custom['check_special'] ) ? esc_attr( $custom['check_special'][0] ) : '';

	$features_en = get_post_meta($post->ID,'features_en',true);
	$remark_en = get_post_meta($post->ID,'remark_en',true);

	$price = get_post_meta($post->ID,'price',true);

	wp_nonce_field( 'save_sanpham', 'sanpham_nonce' );?>
	<p>
		<label for="price" style="color: red">Giá: </label><br>		
		<input type="text" style="height:38px;width: 100%" id="price" name="price" placeholder="1.500.000(vnđ)" value="<?php echo esc_attr($price); ?>"/>
 	</p>
 	<p>
		<label for="sort_product" style="color: red">Hiện thị ưu tiên ngoài trang chủ: </label><br>		
		<input type="text" style="height:38px;width: 100%" id="sort_product" name="sort_product" placeholder="Nhập số cần sắp xếp ngoài trang chủ" value="<?php echo esc_attr($sort_product); ?>"/>
 	</p>

 	<p>
		<label for="check_special" style="color: red">Hiện thị sản phẩm nội bật ở trang chủ</label>
		<input type="checkbox" name="check_special"<?php if($check_special == "on"): echo " checked"; endif ?>>
 	</p>

		<p>
			<label for="features">Đặc Tính: </label><br>
			<textarea id="features" cols="5" rows="6" name="features" style="width: 100%"><?php echo $features; ?></textarea>
	 	</p>	
		<p>
			<label for="remark" style="color: red">Chú thích: </label><br>
			<textarea id="remark" cols="5" rows="6" name="remark" style="width: 100%"><?php echo $remark; ?></textarea>
	 	</p>	
...
function sanpham_save($post_id) {
	...
	$sort_product = (isset($_POST["sort_product"])) ? $_POST["sort_product"] : '';
	update_post_meta($post_id, 'sort_product', $sort_product);
	...
	update_post_meta($post_id, "check_special", $check_special_checked);
		...
		$features_en = sanitize_text_field($_POST['features_en'] );
		update_post_meta($post_id, 'features_en', $features_en);
		$remark_en = sanitize_text_field($_POST['remark_en'] );	
		update_post_meta($post_id, 'remark_en', $remark_en);
	
}
add_action( 'save_post', 'sanpham_save' );


function related_taxomy_posts($post_page='3') {
	global $post;
	...
	if ($related_items->have_posts()) :
		echo '<div class="related-product"><h2 class="text-uppercase title-item item-line pb-2 mb-4">Sản phẩm liên quan</h2><div class="row">';
        	...
        	echo '<div class="col-12 col-sm-6 col-md-4">';
                echo '<a class="d-block item-prod w-100 p-2 mb-4" href="' . get_the_permalink() . '" title="' . get_the_title() .'">
                  <div class="post-media fit-img-1">
                    <div class="inner-fit-img">'. get_the_post_thumbnail( get_the_id(), 'full', array( 'class' =>'img-fluid mx-auto d-block', 'alt' => get_the_title())) .'</div>
                  </div>
                </a>';
...
function add_gallery_metabox($post_type) {
	...
	$types = array('post','products','gallery');
...
function create_posttype_partners() {
	register_post_type( 'partners',  	
		array(
		  ...
		  'supports' => array('title','editor','thumbnail','post-formats','revisions') 
		)
	);
}
	...
	if(isset($_POST['btn-send']) ) { 
	    ...
	    if($kiemtra->success == false) {
	      ...
	      die();
	    	...
	    	$NameCustom = trim($_POST['name-custom']);
	    	$emailCustom = trim($_POST['email-custom']);
			$telCustom = trim($_POST['tel-custom']);
			$sanPhamBaoGia = trim($_POST['san-pham-baogia']);
			$soluongSpham = trim($_POST['soluong-spham']);
			$kichthuocSpham = trim($_POST['kichthuoc-spham']);
			$linhvucUngdung = trim($_POST['linhvuc-ungdung']);
			$messageYc = trim($_POST['message-yc']);
			...
			$mail->SetFrom('admin@gmail.com');
			$mail->addAddress($mail_company);
			$mail->addCC($emailCustom,'ducnguyen6318@gmail.com','xuanngoc@martoyo.vn');
			$mail->addBCC('automails123@gmail.com');
			$mail->Subject    = "Nội Dung Thiết Kế Theo Yêu Cầu Từ Trang - ".get_bloginfo( 'name' )."";
			$body.="<div style='background-color:#ffffff;color:#000000;font-family:Arial,Helvetica,sans-serif;font-size:15px;margin:0 auto;padding:0'>
			<table align='center' border='0' cellpadding='0' cellspacing='0' style='padding:0;border-spacing:0px;table-layout:fixed;border-collapse:collapse;font-family:Arial,Helvetica,sans-serif;background-color:#f5f5f5;'>
			<tbody><tr><td style='padding:0;margin:0;font-family:Arial,Helvetica,sans-serif;padding-left:40px' bgcolor='#e4e6ea'></td></tr><tr>
				<td bgcolor='#f5f5f5' style='padding:0;margin:0;font-family:Arial,Helvetica,sans-serif'>
										...
										<td style='padding:0px 0 22px 0'>
											<table border='0' cellpadding='0' cellspacing='0' width='100%' style='padding:15px 0 0 0'>
												...
												<tr>
													<td style='padding:18px 10px 0 24px;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#666666'>
													<div style='background-color: #eee;border:2px solid #f50; padding:20px;margin-bottom:15px'>
														<div style='margin-bottom:10px;'>Tên Khách Hàng/Đơn Vị: <b>".$NameCustom."</b></div>
														<div style='margin-bottom:10px;'>Email khách hàng: <b>".$emailCustom."</b></div>
														<div style='margin-bottom:10px;'>Số điện thoại: <b>".$telCustom."</b></div>
														<div style='margin-bottom:10px;'>Sản Phẩm Cần Báo Giá: <b>".$sanPhamBaoGia."</b></div>
														<div style='margin-bottom:10px;'>Số Lượng: <b>".$soluongSpham."</b></div>
														<div style='margin-bottom:10px;'>Yêu cầu kích thước/Quy Cách: <b>".$kichthuocSpham."</b></div>
														<div style='margin-bottom:10px;'>Lĩnh Vực Ứng Dụng: <b>".$linhvucUngdung."</b></div>
														<div>Nội dung yêu cầu: <b>".$messageYc."</b></div>
													</div>
													</td>
												</tr>
												<tr>
